...
class DIT(nn.Module, huggingface_hub.PyTorchModelHubMixin):
    def __init__(self, config, vocab_size: int):
        ...
        self.extra_cond_dim = getattr(config.model, "extra_cond_dim", None)
        self.extra_cond_merge = getattr(config.model, "extra_cond_merge", "sum")

        if self.extra_cond_dim is not None:
            if self.extra_cond_merge == "sum":
                self.extra_cond_proj = nn.Linear(self.extra_cond_dim, self.cond_dim, bias=True)
                self.extra_cond_mlp = None
            elif self.extra_cond_merge == "concat_mlp":
                self.extra_cond_proj = None
                self.extra_cond_mlp = nn.Sequential(
                    nn.Linear(self.cond_dim + self.extra_cond_dim, self.cond_dim),
                    nn.SiLU(),
                    nn.Linear(self.cond_dim, self.cond_dim),
                )
            else:
                raise ValueError(f"Unknown extra_cond_merge: {self.extra_cond_merge}")
        else:
            self.extra_cond_proj = None
            self.extra_cond_mlp = None
    ...
    def forward(self, indices, sigma, extra_cond=None):
        x = self.vocab_embed(indices)
        c_time = F.silu(self.sigma_map(sigma))

        if extra_cond is not None:
            extra = extra_cond.to(dtype=c_time.dtype)
            assert extra.device == c_time.device
            if self.extra_cond_proj is not None:
                c = c_time + self.extra_cond_proj(extra)
            elif self.extra_cond_mlp is not None:
                c = self.extra_cond_mlp(torch.cat([c_time, extra], dim=-1))
            else:
                c = c_time
        ...
        with torch.autocast(
            device_type="cuda",
            dtype=torch.bfloat16 if _cuda_sm() >= 80 else torch.float32,
        ):
