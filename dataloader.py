...
from generation.data.dataset import ShardDataset  # noqa: E402
from generation.data.utils import (  # noqa: E402
    get_collator as get_seq_collator,
    get_latent_dataconf,
    get_transforms,
)
...
class PipelineCollator:
    def __init__(self, base_collator, *postprocessors):
        self.base_collator = base_collator
        self.postproc = list(postprocessors)

    def __call__(self, seqs):
        out = self.base_collator(seqs)  # GenBatch
        for p in self.postproc:
            out = p(out)
        return out
...
def get_dataloaders(outter_data_conf: DataConfig, seed: int):
    ...
    internal_dataconf = get_latent_dataconf(train_collator.base_collator, data_conf)
    # Create loaders
