...
class Diffusion(L.LightningModule):
    ...
    def _ddpm_caching_update(self, x, t, dt, p_x0=None, locked_mask=None):
        ...
        if locked_mask is None:
            ...
            copy_flag = ((x != self.mask_index) | locked_mask).to(x.dtype)
    ...
    def sample_subs_guidance(
        ...
        locked_mask = None
        target = None
        for stride in range(num_strides + 1):
            p_x0_cache = None
            x = self._sample_prior(n_samples, self.config.model.length).to(self.device)

            if stride == 0 and prefix_ids is not None and len(prefix_ids) > 0:
                L = self.config.model.length
                pref = torch.as_tensor(prefix_ids, device=x.device, dtype=x.dtype)[:L]
                x[:, : pref.shape[0]] = pref

                locked_mask = torch.zeros_like(x, dtype=torch.bool)
                locked_mask[:, : pref.shape[0]] = True
            ...
            if locked_mask is not None:
                # не трогаем залоченные позиции (префикс + левый контекст окна)
                x = torch.where(locked_mask, x, x_hat)
            else:
                x = x_hat
    ...
    def restore_model_and_semi_ar_sample(
        self, stride_length, num_strides, dt=0.001, prefix_ids=None
    ):
        """Generate samples from the model."""
        # Lightning auto-casting is not working in this method for some reason
        if self.ema:
